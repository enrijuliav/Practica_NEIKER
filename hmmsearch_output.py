...
def prepare_db(file: str) -> list[list[str]]:
    """
    Esta función transforma los archivo de bases de datos proteicas de NcycFunGen en un objeto parseado y empleable
...
phmmernirS = extract_NIH_ID("nirS_phmmer.txt", "enrijuliav@gmail.com")
print(phmmernirS[0])
print(phmmernirS[1])

with open("phmmer_nirK.txt", "w") as f:     
    for ID in phmmernirK[0]:
        f.write(f">{ID}\n")
        f.write(phmmernirK[0][ID]+"\n")

with open("phmmer_nirS.txt", "w") as f:
    for ID in phmmernirS[0]:
        f.write(f">{ID}\n")
        f.write(phmmernirS[0][ID]+"\n")

with open("genus_nirK.txt", "w") as f:     
    for ID in phmmernirK[1]:
        f.write(f">{ID}\n")
        f.write("; ".join(phmmernirK[1][ID])+"\n")

with open("genus_nirS.txt", "w") as f:
    for ID in phmmernirS[1]:
        f.write(f">{ID}\n")
